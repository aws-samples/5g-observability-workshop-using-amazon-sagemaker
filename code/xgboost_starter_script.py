...
if __name__ == "__main__":
    ...
    with load_run(sagemaker_session=session) as run:
        ...
        train_std = cv_results.iloc[-1]['train-auc-std']
        valid_auc = cv_results.iloc[-1]['test-auc-mean']
        valid_std = cv_results.iloc[-1]['test-auc-std']
        
        print(f"[0]#011train-auc:{train_auc}")
        print(f"[1]#011train-auc std:{train_std}")
        print(f"[2]#011validation-auc:{valid_auc}")
        print(f"[3]#011validation-auc std:{valid_std}")
        
        run.log_metric(name="Train-AUC", value=train_auc)
        run.log_metric(name="Train-Std", value=train_std)
        
        run.log_metric(name="Validation-AUC", value=valid_auc)
        run.log_metric(name="Validation-Std", value=valid_std)

        metrics_data = {
            "binary_classification_metrics": {
                "validation:auc": {
                    "value": train_auc,
                    "standard_deviation": train_std
                },
                "train:auc": {
                    "value": valid_auc,
                    "standard_deviation": valid_std
                },
            }
        }
